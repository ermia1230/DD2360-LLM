...
void gpt2_forward(GPT2 *model, int* inputs, int* targets, int B, int T) {
    ...
    if (targets != NULL) {
        ...
        // For FP16: we'll handle loss scaling in the optimizer, so keep this simple
...
// Loss scaling kernel - scale down gradients after backward pass
// FP16 needs loss scaling: scale loss up during forward, scale gradients down during backward
__global__ void gradient_scale_kernel(__half* grads, long num_parameters, float scale) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= num_parameters) return;
    
    float grad = __half2float(grads[i]);
    grad *= scale;  // Scale down gradient (loss was scaled up)
    // Also clip to prevent overflow in FP16 (range: ±65504)
    grad = fmaxf(-65000.0f, fminf(65000.0f, grad));
...
void gpt2_update(GPT2 *model, float learning_rate, float beta1, float beta2, float eps, float weight_decay, int t) {
    ...
    // Scale gradients down (if loss was scaled up by loss_scale, divide gradients by loss_scale)
    // For FP16, typical loss_scale is 512-2048. Start conservative at 128.
    float loss_scale = 128.0f;
    gradient_scale_kernel<<<num_blocks, block_size>>>(model->grads_memory, model->num_parameters, 1.0f / loss_scale);
    cudaCheck(cudaGetLastError());
...
int main(int argc, char *argv[]) {
    ...
    for (int step = 0; step <= train_num_batches; step++) {
        ...
        gpt2_zero_grad(&model);
        gpt2_backward(&model);
        // FP16 often needs slightly lower learning rate (0.7-0.8x of FP32)
        float fp16_lr = learning_rate * 0.75f;
        gpt2_update(&model, fp16_lr, 0.9f, 0.999f, 1e-8f, 0.0f, step+1);
